...
router.post("/loans", async (req, res) => {
  ...
  if (userError || !userData) {
    console.error("Chyba při ověřování role:", userError);
    return res.status(500).json({ message: "Chyba při ověřování oprávnění" });
  }

  // Zkontrolujeme, zda má roli knihovník (případně uprav řetězec podle toho, jak to máš v DB)
  if (userData.role !== "vedouci") {
    return res.status(403).json({ message: "Pouze knihovník může vytvářet výpůjčky žákům." });
  }
