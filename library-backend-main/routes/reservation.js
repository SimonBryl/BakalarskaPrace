...
router.post("/res", async (req, res) => {
  ...
  try {
    const { data: availableBooks, error: availableError } = await supabase
      .from("Books")
      .select("id_kniha")
      .eq("nazev", nazev_knihy)
      .eq("stav", "kdispozici"); 

    if (availableError) throw availableError;
    
    if (availableBooks && availableBooks.length > 0) {
      return res.status(400).json({ message: "Tento titul je fyzicky dostupný v knihovně k vypůjčení. Rezervace není nutná." });
    }

    //  Zjistíme všechny ID knih, které má uživatel právě půjčené a ještě je nevrátil
    ...
    //  Pokud má nějaké aktivní výpůjčky, ověříme, jestli se některá nejmenuje stejně jako ta, kterou chce rezervovat
    if (activeLoans && activeLoans.length > 0) {
      const borrowedBookIds = activeLoans.map(loan => loan.id_kniha);

      const { data: matchingBorrowedBooks, error: matchError } = await supabase
        .from("Books")
        .select("id_kniha")
        .in("id_kniha", borrowedBookIds)
        .eq("nazev", nazev_knihy);

      if (matchError) throw matchError;
      
      if (matchingBorrowedBooks && matchingBorrowedBooks.length > 0) {
        return res.status(400).json({ message: "Tuto knihu již máte aktuálně vypůjčenou." });
      }
    }
